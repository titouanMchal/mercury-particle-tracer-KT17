...
class Shoot:
    ...
    def compute_trajectory_boris(
        self,
        E: callable = KT_field.E_KT,
        B: callable = KT_field.B_KT,
        precision=360,
        adaptative_dt=False,
        direction=1,
        dse=1e4,
        ...
        """
        This method computes the position and velocity of the particle over time in an electromagnetic field
        using the Boris-Buneman integrator (symplectic and energy-conserving scheme)
        ...
        pdrop : float, optional
            Half electrostatic potential drop (in volts) used in electric field model. Default is 10E3.
        ...
        n_print = np.floor(self.N / 100)

        if E != False:
            self.p, self.e1, self.e2, E_0 = KT_field.E_KT(self.x[0], pdrop=pdrop)
        ...
        for i in range(1, self.N):
            ...
            self.B[i - 1] = B(self.x[i - 1])

            if np.any(np.isnan(self.B[i - 1])):
                print("\nSimulation ended : B not defined")
                break
            ...
            if E != False:
                if curvilinear_distance >= dse:
                    ...
                    result = KT_field.E_KT(self.x[i - 1], pdrop=pdrop)

                    if result is None:
                        print("\nSimulation ended : E computation failed")
                        break
...
def push(
    ...
    b0 = KT_field.B_KT(x0_RM)
    u1, u2 = f.vec_ortho(KT_field.B_KT(x0_RM))
